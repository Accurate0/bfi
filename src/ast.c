...
static inline void ast_optimise_loop_node(node_t *node, list_t *loop,
                                          ast_command_t opt) {
  switch (opt) {
...
bf_t *ast_init(bool optimisations) {
  ...
  for (int8_t c = io_read_next(); c != EOF; c = io_read_next()) {
    ...
    // simplify ++/-- and >>/<< instructions into a single instruction and
    // count;
...
static void ast_dump(node_t *node) { ast_dump_internal(node, 0); }
